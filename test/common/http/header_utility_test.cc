...
string_match:
  ...
  auto matcher = std::make_unique<HeaderUtility::HeaderData>(parseHeaderMatcherFromYaml(yaml), context_);
...
TEST_F(MatchHeadersTest, MatchesHeadersIndividuallyPresentMatch) {
  ...
  auto make = [&](const std::string& yaml) {
    return std::make_unique<HeaderUtility::HeaderData>(parseHeaderMatcherFromYaml(yaml), context_);
  };
