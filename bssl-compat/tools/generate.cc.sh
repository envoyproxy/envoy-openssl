...
CMAKEFILE="$TOP_DIR/CMakeLists.txt"
...
[[ -f "$CMAKEFILE" ]] || error "CMAKEFILE $CMAKEFILE does not exist"

################################################################################
# Find out which header file the function is declared in
################################################################################
HDR_FILE=$(grep "OPENSSL_EXPORT.*[^A-Za-z0-9_]$FUNC_NAME[ \t]*(" $INCLUDE_DIR/openssl/* | cut -d: -f1)
if [ ! -f "$HDR_FILE" ]; then
...
FUNC_SIG_RET="$(echo "$FUNC_SIG_ONE_LINE" | sed -e "s/\(.*[^A-Za-z_]\)$FUNC_NAME.*$/\1/g")"
FUNC_SIG_ARGS="$(echo "$FUNC_SIG_ONE_LINE" | sed -e "s/.*$FUNC_NAME//g")"
FUNC_SIG_ARGS_NAMES="($(echo $FUNC_SIG_ARGS | sed 's/[()]//g' | tr ',' '\n' | sed -e 's/^.*[^a-zA-Z0-9_]\([a-zA-Z_][a-zA-Z0-9_]*\)/\1/g' | tr '\n' ',' | sed -e 's/,$//g' -e 's/,/, /g' -e 's/(void)/()/g'))"
